...
| 比卡的冒險 | `rpg` | 文字冒險 RPG：故事線選單（星空冒險已上線，經典奇幻／幽默日常為敬請期待佔位），劇情分支＋回合制戰鬥（攻擊／防禦／道具／逃跑），HP／攻擊力／背包道具，選項可帶 `require`（有道具才出現）與 `effect`（扣血、拿道具），多重結局；進度每節點自動存 localStorage（`bikaRpgSave`），達成結局記入 `bikaRpgEndings` 做結局收集並清該線進度；劇情文字有打字機效果（點文字可跳過） |
...
- 卡片封面：前四張用圖片（同作品集卡片的 `.card-preview-img` 模式）——猜單字 `assets/Wordle.png`、小精靈 `assets/PAC_MAN.png`、圈圈叉叉 `assets/OOXX.png`、知識王 `assets/Knowledge.png`；比卡的冒險暫用 `.rpg-bg` 漸層底＋🌌 emoji（之後可補 `assets/RPG.png` 換成圖片模式）；`.wordle-bg` 等漸層底與 `.game-emoji` 樣式仍留在 CSS 裡當底色/備用
- **新增知識王題目步驟**：在 `assets/games/quiz.json` 加一筆 `{ "q": { "zh", "en" }, "options": [4 個 { "zh", "en" }], "answer": 正解索引 }`（選項顯示時會自動洗牌，answer 填在原陣列的索引即可）

#### 文字 RPG（比卡的冒險）劇情資料格式
- 故事線設定在 `js/games.js` 的 `RPG_LINES`（emoji、名稱、簡介、`endings` 結局數、`available` 是否開放、`file` 劇情檔路徑）
...
- **新增故事線步驟**：寫一個新的劇情 JSON 放進 `assets/games/rpg/`，在 `RPG_LINES` 把該線 `available` 改 `true` 並填 `file` 與 `endings` 數
...
- [ ] 筆記內容繼續新增其他科目筆記
- [ ] 文字 RPG 補上「經典奇幻」「幽默日常」故事線（寫新劇情 JSON + `RPG_LINES` 開 `available`）
- [ ] 文字 RPG 卡片封面補一張 `assets/RPG.png`（目前用漸層底 + 🌌 emoji）
- [ ] 音樂作品繼續新增歌曲
- [ ] 畫作持續新增
- [ ] 小教室新增單字（手動更新 JSON 檔）
- [ ] 加入更多備審文件
